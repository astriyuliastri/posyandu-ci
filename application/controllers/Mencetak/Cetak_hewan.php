...
class Cetak_hewan extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('mencetak/cetak_hewan', $data);
    }
    ...
    public function cetak()
    {
        if (isset($_GET['filter']) && !empty($_GET['filter'])) { // Cek apakah user telah memilih filter dan klik tombol tampilkan
            ...
            if ($filter == '1') { // Jika filter nya 1 (per tanggal)
                ...
                $title = 'Laporan Vaksin Hewan<br> Dalam Wilayah Kerja Posyandu <br> Tanggal ' . date('d-m-y', strtotime($tgl_skrng));
                ...
                $title = 'Laporan Vaksin Hewan<br> Dalam Wilayah Kerja Posyandu <br> Bulan ' . $nama_bulan[$bulan] . ' ' . $tahun;
                $cetak_hewan = $this->Cetak_hewan_model->view_by_month($bulan, $tahun); // Panggil fungsi view_by_month yang ada di Cetak_hewan_model
            } else { // Jika filter nya 3 (per tahun)
                $tahun = $_GET['tahun'];

                $title = 'Laporan Vaksin Hewan <br> Dalam Wilayah Kerja Posyandu <br>Tahun ' . $tahun;
                $cetak_hewan = $this->Cetak_hewan_model->view_by_year($tahun); // Panggil fungsi view_by_year yang ada di Cetak_hewan_model
            }
        } else { // Jika user tidak mengklik tombol tampilkan
            $title = 'Laporan Vaksin Hewan<br> Dalam Wilayah Kerja Posyandu <br>';
            $cetak_hewan = $this->Cetak_hewan_model->view_all(); // Panggil fungsi view_all yang ada di Cetak_hewan_model
        }
